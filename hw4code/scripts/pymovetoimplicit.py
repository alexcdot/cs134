...
def tplink_callback(msg):
    """x: 0.0
    y: 0.0
    width: 0.0
    height: 0.0"""
    send_x_y_to_yaw_pitch_callback(msg.x, msg.y)
...
def faces_callback(msg):
    ...
    if len(msg.faces) > 0:
        ...
        send_x_y_to_yaw_pitch_callback(face.x, face.y)


def send_x_y_to_yaw_pitch_callback(x, y):
    ...
    msg.yaw = yaw - (x - IMAGE_WIDTH / 2) / PIXELS_PER_RADIAN
    msg.pitch = pitch + (y - IMAGE_HEIGHT / 2) / PIXELS_PER_RADIAN
...
if __name__ == "__main__":
    ...
    tplink_sub = rospy.Subscriber('/detector/tplink', Rect, tplink_callback)
    rospy.sleep(0.4)
